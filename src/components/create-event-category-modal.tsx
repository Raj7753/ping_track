...
const ICON_OPTIONS: { key: keyof typeof Icons; label: string }[] = [
  { key: "payment", label: "Money (Sale)" },
  { key: "userSignup", label: "User (Sign-up)" },
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-3">
                {ICON_OPTIONS.map(({ key, label }) => {
                  const IconComp = Icons[key] as React.ComponentType<{ className?: string }>
                  return (
                    <button
                      key={key}
                      type="button"
                      title={label}
                      className={cn(
                        "size-10 flex items-center justify-center rounded-md transition-all",
                        selectedEmoji === key
                          ? "bg-brand-100 ring-2 ring-brand-700 scale-110"
                          : "bg-brand-100 hover:bg-brand-200"
                      )}
                      onClick={() => setValue("emoji", key)}
                    >
                      <IconComp className="size-5 text-brand-700" />
                    </button>
                  )
                })}
              </div>
